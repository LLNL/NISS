...
def main(smoother_size=3, num_steps=8000, do_print=True, do_plot=True):
    # centrosymmetric
    ...
    for test in [test_smoother_optimal, test_smoother_nn1, test_smoother_nn2]:
        try:
            ...
            if do_print:
                print(f'* Inf-norm Smoothing factor = {inf_smooth_factor:.8e}')
                print(f'* Measured Smoothing factor = {smooth_factor:.8e}')
            # plot
            if do_plot:
...
if __name__ == "__main__":
    sys.exit(main(smoother_size=3, num_steps=2000)[0])
